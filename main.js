...
const searchCounter = document.getElementById('searchCounter'); // 카운터 요소 추가

// 검색 횟수 관리
let searchCount = parseInt(localStorage.getItem('searchCount') || '1000', 10);

/**
 * 화면의 검색 횟수 카운터를 업데이트하는 함수
 */
function updateCounterDisplay() {
    searchCounter.innerHTML = `지금까지 <span class="font-bold text-blue-600">${searchCount}</span>번 나이를 검색했어요.`;
}
...
searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
        e.preventDefault(); // Enter 키의 기본 동작(폼 제출 등) 방지
        handleSearch();
    }
});
...
async function handleSearch() {
    if (isSearching) return; // 이미 검색 중이면 실행하지 않음
